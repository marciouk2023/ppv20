...
interface SendMessageOptions {
  ...
  // Adicionar um campo para identificação única da mensagem (opcional)
  uniqueId?: string
}
  ...
  try {
    ...
    const result = await runTransaction(db, async (transaction) => {
      ...
      if (docSnap.exists() && docSnap.data()?.status !== "failed") {
        // Check if exists and wasn't marked as failed previously
        console.warn(
      ...
      transaction.set(
        sentMessageDocRef,
        {
          // Use set with merge=true or handle update explicitly if needed
          messageId,
          phoneNumber,
          contactId,
          contactName,
        ...
        { merge: true },
      ) // Use merge to handle potential retries of failed attempts
      ...
      console.log(
        `[sendMessage_LOG] Transaction: Prepared WAHA Request - Endpoint: ${wahaEndpoint}, Session: ${sessionName}`,
      )
    ...
    if (result && result.success) {
      if (result.duplicated) {
        console.log(
          `--- [sendMessage_LOG] Finished processing (DUPLICATE in TX) in ${Date.now() - functionStartTime}ms ---`,
        )
      } else {
