...
export default function ConfiguracoesPage() {
    ...
    try {
      ...
      if (sessionInfo.hasSession && sessionInfo.sessionName) {
        ...
        if (
          ...
          try {
            const statusEndpoint = `/api/whatsapp/sessions/${sessionInfo.sessionName}/status`
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const startSessionResponse = await fetch(`/api/whatsapp/generate-qr`, {
        method: "POST",
  ...
  const startStatusChecking = (sessionNameToCheck: string) => {
    ...
    statusCheckInterval = setInterval(async () => {
      ...
      try {
        ...
        const statusEndpoint = `/api/whatsapp/sessions/${wahaSessionName}/status`
